...
RegisterNetEvent('QRCore:Client:OnPlayerUnload', onLogout)

RegisterNetEvent('QRCore:Player:SetPlayerData', function(data)
...
function client.setPlayerStatus(values)
	for name, value in pairs(values) do
		...
		if name == "hunger" then
			TriggerServerEvent('consumables:server:addHunger', QRCore.Functions.GetPlayerData().metadata.hunger + value)
		elseif name == "thirst" then
			TriggerServerEvent('consumables:server:addThirst', QRCore.Functions.GetPlayerData().metadata.thirst + value)
...
AddEventHandler(('__cfx_export_qr-inventory_HasItem'), function(setCB)
	setCB(hasItem)
end)
