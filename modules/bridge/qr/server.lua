...
AddEventHandler('QRCore:Server:OnPlayerUnload', playerDropped)

AddEventHandler('QRCore:Server:OnJobUpdate', function(source, job)
...
AddEventHandler('QRCore:Server:OnGangUpdate', function(source, gang)
	local inventory = Inventory(source)
	if not inventory then return end
...
AddEventHandler('onResourceStart', function(resource)
	if resource ~= 'qr-weapons' or resource ~= 'qr-shops' then return end
...
local function setupPlayer(Player)
	...
	QRCore.Functions.AddPlayerMethod(Player.PlayerData.source, "GetItemByName", function(itemName)
		return setItemCompatibilityProps(Inventory.GetSlotWithItem(Player.PlayerData.source, itemName))
	end)

	QRCore.Functions.AddPlayerMethod(Player.PlayerData.source, "GetItemsByName", function(itemName)
	...
	QRCore.Functions.AddPlayerMethod(Player.PlayerData.source, "SetInventory", function()
...
AddEventHandler('QRCore:Server:PlayerLoaded', setupPlayer)

SetTimeout(500, function()
	QRCore = exports['qr-core']:GetCoreObject()
	server.GetPlayerFromId = QRCore.Functions.GetPlayer
	local weapState = GetResourceState('qr-weapons')

	if weapState ~= 'missing' and (weapState == 'started' or weapState == 'starting') then
		StopResource('qr-weapons')
	end

	local shopState = GetResourceState('qr-shops')

	if shopState ~= 'missing' and (shopState == 'started' or shopState == 'starting') then
		StopResource('qr-shops')
	end

	for _, Player in pairs(QRCore.Functions.GetQRPlayers()) do setupPlayer(Player) end
end)

function server.UseItem(source, itemName, data)
	local cb = QRCore.Functions.CanUseItem(itemName)
	return cb and cb(source, data)
end

AddEventHandler('QRCore:Server:OnMoneyChange', function(src, account, amount, changeType)
...
local function exportHandler(exportName, func)
    AddEventHandler(('__cfx_export_qr-inventory_%s'):format(exportName), function(setCB)
        setCB(func)
    end)
end
